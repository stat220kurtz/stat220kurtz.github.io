...
```{r}
head(scaled_cancer$Compactness)
```

. . . 

Standardized `Compactness` from created `standardize()` function
...
```{r}
std_mass <- standardize(penguins$body_mass_g, na.rm = TRUE)
head(std_mass)
```
...
```{r}
std_mass <- standardize(penguins$body_mass_g, na.rm = FALSE)
head(std_mass)
```
...
```{r eval=FALSE}
std_compact <- standardize(penguins$body_mass_g)
```
...
```{r}
std_mass <- standardize(penguins$body_mass_g)
std_mass
```
...
::: task
Write a plotting function  called `scatterplot()` that makes a scatterplot of any two quantitative variables, coloring the points by a 3rd categorical variable. 
