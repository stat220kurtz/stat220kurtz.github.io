...
2.  Take a look at the first 3 pairs of data (Example 1 through Example 3) together and talk through (conceptually! in words! no R code!) how you think they should be combined
:::
...
```
:::
Why would that have posed a problem?
:::::
...
Keep all rows from `x` where there are matching values in `y`, return all combinations of multiple matches if multiples exist
...
## ## {background-image="../img/join_diagram.png" background-size="30%"}
...
Last week, we tidied data containing ratings from *The Great British Bakeoff*.
...
tidy_ratings %>%
  full_join(episodes, by = c("series", "episode")) %>%
  slice_max(viewers, n=10)
```
