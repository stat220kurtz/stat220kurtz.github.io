...
:::{.column .nonincremental .fragment width="35%"}
Example questions: 
...
- `start` / `end` = position of the first and last characters
  - Note: positive numbers start from the left and negative numbers start from the right
...
```{r}
# extract more than one string by setting start and end as vectors
# match the elements of each vector to each string desired
...
How does the proportion of newborns with names that end with a vowel change over time?
...
Use the `courses` dataset in the activities file to answer the following:

(a) How many of the course numbers end in `.00`? Use `str_detect()` or `str_count()` to help you answer this question.

```{r, eval=FALSE, echo=FALSE}
courses <- read_csv("https://stat220kurtz.github.io/data/winter2023_course_tbl.csv", col_types = list(course = col_character()))

sum(str_detect(courses$course, pattern="\\.00"))
sum(str_detect(courses$course, "\\.00$"))

courses <- courses %>%
  mutate(section = str_sub(course, -2, -1))

sum(str_detect(courses$title, pattern="Introduction"))

courses[which(str_length(courses$title)==102),]
courses[which(str_length(courses$title)==6),]

courses <- courses %>%
  mutate(words = str_split(title, " ")) %>%
  mutate(num_words = lengths(words))

str_subset(courses$title, "!")
```
...
(e) Which course name is comprised of the most words? To do this, create a new column containing the words in each title using `mutate()` and `str_split()`. Then, create another column calculating the `lengths()` of the values in column you just created.


(f) Use `str_subset()` to return the course names that contain exclamation points (`!`).
:::

Note: Some of these might be tricky. That's okay! Try them and move on if you're stuck. We will learn more tools after this activity and you can revisit.
...
- Sometimes the patterns we wish to detect, extract, etc. too complex for exact matching 
    ...
    + Extract the string that comes after the dash (e.g. `hw01-emilylkurtz`)
...
Find out how many 2015 baby names contain the letter A three times. Use `str_count()`
:::

```{r, echo=FALSE, eval=FALSE}
babynames2015 <- babynames %>% 
  filter(year == 2015) %>%                       # extract year 2015
  mutate(ends_with_vowel = str_extract(name, "[aeiouy]$")) %>%  # create logical column
  count(ends_with_vowel)

babynames2015 <- babynames %>% 
  filter(year == 2015)
matches <- str_count(babynames2015$name, "[Aa]") == 3
babynames2015$name[matches]
```
